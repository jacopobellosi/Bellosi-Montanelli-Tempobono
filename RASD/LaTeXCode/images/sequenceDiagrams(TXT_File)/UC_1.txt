...
activate App
activate 
App->>Server: registerUser(username, email, password)
activate Server

    note right of Server: Server performs validation and checks uniqueness.
    
    alt Credentials Invalid (e.g., Email taken)
        Server-->>App: 400_BadRequest("Email or username already in use")
        App->>User: showError("Email or username already in use")
        
    else Credentials Valid
        note right of Server: Creates new user account and generates session token.
        Server-->>App: 201_Created(sessionToken)
        deactivate Server
