...
\begin{figure}[H]
    \begin{center}
        \includegraphics[width=1\linewidth]{RASD/LaTeXCode/images/sequenceDiagrams/UC_6.png}
...
\begin{center}
    \begin{longtable}{|l|p{0.75\linewidth}|}
        ...
        \textbf{Event Flow}       & 1 - The User taps the "Start Recording" button. \\
                                  & 2 - BBP Application requests permissions for GPS and Motion Sensors, if not already granted. \\
                                  & 3 - BBP Application activates \texttt{Device Sensors} (GPS, Accelerometer, Gyroscope) [R22]. \\
                                  & 4 - The app displays the recording HUD, showing real-time statistics (speed, distance, time) [R21]. \\
                                  & 5 - \textbf{In parallel:} The app records the user's \texttt{GPS\_points} sequence (R20) and monitors sensor data. \\
                                  [cite_start]& 6 - \textbf{If} sensor data exceeds the predefined threshold, the app internally logs a new \texttt{sensor\_event} (R23) [cite: 94-95]. \\
                                  & 7 - The User taps the "Stop Recording" button (R24). \\
                                  & 8 - BBP Application stops all sensors and finalizes the path geometry. \\
                                  [cite_start]& 9 - BBP Application queries the \texttt{External Weather Service} and retrieves the weather conditions for the trip (R29) [cite: 86-87]. \\
                                  [cite_start]& 10 - BBP Application calculates the final \texttt{Statistics} (avg speed, total distance, etc.) (R28) [cite: 85-88]. \\
                                  & 11 - The system checks if any \texttt{sensor\_event}s were logged (R25). \\
                                  & 12 - \textbf{If} events were logged, the flow is extended by \textbf{`Review Detections`} (UC11). \\
                                  & 13 - \textbf{If} no events were logged (or after review is complete), the system displays the "Save Trip" screen, showing the summary, stats, and weather. \\
                                  & 14 - The User triggers \textbf{`Save Trip/Path`} (UC9). \\
        \hline
        \textbf{Exit condition}   & The user has completed the recording. The system proceeds to the review or save flow. \\
        \hline
        \textbf{Exceptions}       & \begin{itemize}[noitemsep, nolistsep]
                                    \item The user discards the trip instead of saving. The recorded data (\texttt{GPS\_points}, \texttt{sensor\_event}s) is discarded.
                                    \item GPS signal is lost (violates DA2). The system should notify the user that tracking is paused (Not explicitly handled per DA2, but good to note).
                                  \end{itemize}\\
        \hline
