...
\subsubsection*{UC7. Record Trip (Automatic)}
\begin{center}
    \begin{longtable}{|l|p{0.75\linewidth}|}
        \hline
        \textbf{Actor}            & Registered User, Device Sensors (GPS, Accelerometer, Gyroscope), External Weather Service \\
        \hline
        \textbf{Entry conditions} & The user is registered, logged in, and on the main map screen. \\
        \hline
        \textbf{Event Flow}       & 1 - The User taps the "Start Recording" button. \\
                                  & 2 - BBP Application requests permissions for GPS and Motion Sensors. \\
                                  & 3 - BBP Application activates \texttt{Device Sensors} [R22]. \\
                                  & 4 - The app displays the recording HUD, showing real-time statistics [R21]. \\
                                  & 5 - \textbf{In parallel:} The app records the user's \texttt{GPS\_points} sequence (R20) and monitors sensor data. \\
                                  [cite_start]& 6 - \textbf{If} sensor data exceeds the threshold, the app internally logs a new \texttt{sensor\_event} (R23) [cite: 94-95]. \\
                                  & 7 - The User taps the "Stop Recording" button (R24). \\
                                  & 8 - BBP Application stops all sensors and finalizes the path geometry. \\
                                  [cite_start]& 9 - BBP Application queries the \texttt{External Weather Service} and retrieves the weather conditions (R29) [cite: 86-87]. \\
                                  [cite_start]& 10 - BBP Application calculates the final \texttt{Statistics} (R28) [cite: 85-88]. \\
                                  & 11 - The system checks if any \texttt{sensor\_event}s were logged (R25). \\
                                  & 12 - \textbf{If} events were logged, the flow is extended by \textbf{`Review Detections`} (UC8). \\
                                  & 13 - \textbf{If} no events were logged (or after review), the system displays the "Save Trip" summary screen, triggering \textbf{`Save Trip`} (UC9). \\
        \hline
        \textbf{Exit condition}   & The user has completed the recording. The system proceeds to the review or save flow. \\
        \hline
        \textbf{Exceptions}       & \begin{itemize}[noitemsep, nolistsep]
...
\begin{figure}[H]
    \begin{center}
        \includegraphics[width=1\linewidth]{RASD/LaTeXCode/images/sequenceDiagrams/UC_7.png}
...
\begin{center}
    \begin{longtable}{|l|p{0.75\linewidth}|}
        ...
        \hline
        \textbf{Event Flow}       & 1 - The BBP Application displays the \texttt{Review Detections} screen. \\
                                  ...
                                  & 7 - The User taps "Finish Review". \\
        \hline
        \textbf{Exit condition}   & All pending \texttt{sensor\_event}s have been processed (confirmed or ignored). The system proceeds to the "Save Trip" screen. \\
        \hline
        \textbf{Exceptions}       & \begin{itemize}[noitemsep, nolistsep]
...
\begin{figure}[H]
    ...
    \end{center}
\end{figure}
\subsubsection*{UC9. Save Trip}
\begin{center}
    \begin{longtable}{|l|p{0.75\linewidth}|}
        \hline
        \textbf{Actor}            & Registered User \\
        \hline
        \textbf{Entry conditions} & The user has completed a \texttt{Record Trip (Automatic)} (UC7) and is on the summary screen. \\
        \hline
        \textbf{Event Flow}       & 1 - BBP Application displays the trip summary (stats, weather, confirmed detections). \\
                                  & 2 - The User enters a name for the \texttt{Trip}. \\
                                  & 4 - The User taps the "Save" button or the "Discard" button. \\
                                  & 5 - If "Save" was pressed, BBP Application sends all \texttt{Trip} data (geometry, stats, weather, reports, name, visibility) to the Server . \\
                                  & 6 - The Server saves the \texttt{Trip} to the user's private archive (R30). \\
                                  & 7 - The Server confirms the save. \\
                                  & 8 - BBP Application displays a "Save successful" message. \\
        \hline
        ...
        \textbf{Exceptions}       & \begin{itemize}[noitemsep, nolistsep]
                                  ...
                                  \end{itemize}\\
        \hline
        \caption{Save Trip use case.}
        \label{tab:uc_save_trip}
    \end{longtable}
\end{center}
\begin{figure}[H]
    \begin{center}
        \includegraphics[width=1\linewidth]{RASD/LaTeXCode/images/sequenceDiagrams/UC_9.png}
        \caption{UC9: Save Trip/Path}
        \label{fig:UC9: Save Trip/Path}%
    \end{center}
\end{figure}
