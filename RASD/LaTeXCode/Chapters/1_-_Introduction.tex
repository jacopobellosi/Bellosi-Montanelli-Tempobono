...
\begin{footnotesize}
    \begin{longtable}{ |l|p{0.5\linewidth}|l|l|}
        ...
        \hline
        SP\csp & The user enters textual data (e.g., credentials, path names, origin, destination). & World (User) & Machine \\
        \hline
        SP\csp & The user interacts with the interface by pressing buttons (e.g., Login, Save, Publish, Search). & World (User) & Machine \\
        \hline
        SP\csp & The user starts the automatic trip recording. & World (User) & Machine \\
        \hline
        SP\csp & The user stops the automatic trip recording. & World (User) & Machine \\
        \hline
        SP\csp & The user manually selects the status of a route (e.g., Optimal, Requires maintenance). & World (User) & Machine \\
        \hline
        SP\csp & The user confirms an automatically detected issue (e.g., pothole or obstacle). & World (User) & Machine \\
        \hline
        SP\csp & The user rejects or corrects an incorrect automatic detection. & World (User) & Machine \\
        \hline
        SP\csp & The user selects a specific route from the displayed results list. & World (User) & Machine \\
        \hline
        SP\csp & The system displays informational or error messages (e.g., Save completed, GPS error). & Machine & World (User) \\
        \hline
        SP\csp & The system shows the interactive map. & Machine & World (User) \\
        \hline
        SP\csp & The system displays the recorded route on the map. & Machine & World (User) \\
        \hline
        SP\csp & The system shows real-time statistics (speed, distance, duration) during the recording. & Machine & World (User) \\
        \hline
        SP\csp & The system displays a summary of trip statistics (average speed, elevation gain, total distance). & Machine & World (User) \\
        \hline
        SP\csp & The system presents a ranked list of routes based on the computed score. & Machine & World (User) \\
        \hline
        SP\csp & The system displays the aggregated (merged) status of a route. & Machine & World (User) \\
        \hline
        SP\csp & The system shows the weather data associated with the route (temperature, wind, conditions). & Machine & World (User) \\
        \hline
        SP\csp & The system displays a confirmation request for an automatic detection (popup / notification). & Machine & World (User) \\
        \hline
        SP\csp & The GPS sensor provides coordinates (latitude, longitude) to the system. & World (GPS Sensor) & Machine \\
        \hline
        SP\csp & The accelerometer sends acceleration vector measurements (x, y, z). & World (Accelerometer) & Machine \\
        \hline
        SP\csp & The gyroscope provides rotation data (x, y, z). & World (Gyroscope) & Machine \\
        \hline
        SP\csp & The external weather service sends JSON data related to temperature, wind, and weather conditions. & World (External weather service) & Machine \\
        \hline
